...
// MARK: - Gacha System
class GashaSystem: ObservableObject {
    ...
    func drawCard(gameState: GameState) -> Bool {
        ...
        guard gameState.spendCoins(drawCost) else {
            return false
        }
...
class GameState: ObservableObject {
    ...
    @Published var rewardAmount: Int = 0
    ...
    // Initialize GameState and load saved coins
    init() {
        self.coins = UserDefaults.standard.integer(forKey: "GameCoins")
    }
    
    // Method to add coins with animation
    ...
    func rewardForSpellingGame() {
        addCoins(10)
    }
    
    func rewardForMemoryGame() {
        addCoins(15)
    }
    
    func rewardForAudioGame() {
        addCoins(20)
    }
    
    // Method to spend coins (for gacha system)
